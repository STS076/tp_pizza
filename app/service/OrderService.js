...
class OrderService {
    ...
    async getMostPopularPizza() {
        ...
        await orders.forEach(order => {
            if (salesByPizza.hasOwnProperty(order.name)) {
                salesByPizza[order.name]++;
            } else {
                salesByPizza[order.name] = 1;
            }
        });
        ...
        for (let quantity in salesByPizza) {
            if (salesByPizza[quantity] > maxSales) {
                maxSales = salesByPizza[quantity];
                mostSold = quantity;
            }
        }
...
console.log("Most popular : ", order.getMostPopularPizza());
